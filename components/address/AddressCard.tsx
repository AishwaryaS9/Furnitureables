...
export default function AddressCard({ address, mode, onEdit, onDelete, onSetDefault, selected = false, onSelect }: AddressCardProps) {
  ...
  // const addressFormatted = `${address.addressLine1}${address.addressLine2 ? `, ${address.addressLine2}` : ""}, ${address.city}, ${address.state} ${address.postalCode}, ${address.country}`;

  return (
    <>
      {/* Schema.org PostalAddress for Search Crawler Structural Context */}
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "streetAddress": `${address.addressLine1}${address.addressLine2 ? ` ${address.addressLine2}` : ""}`,
            "addressLocality": address.city,
            "addressRegion": address.state,
            "postalCode": address.postalCode,
            "addressCountry": address.country,
            "telephone": `${address.phoneCode}${address.phone}`,
          }),
        }}
      />
      ...
      <Card
        ...
        aria-label={`Delivery address for ${address.fullName}, ${address.city}`}
        tabIndex={isSelectMode ? 0 : -1}
        onClick={isSelectMode ? onSelect : undefined}
        onKeyDown={handleKeyDown}
        className={cn(
          "group relative flex flex-col justify-between rounded-2xl border transition-all duration-300 outline-none",
              ...
              <>
                {!address.isDefault && (
                  <Button
                    ...
                    aria-label={`Set default delivery address to ${address.addressLine1}, ${address.city}`}
