...
export default function CartItem({ item }: CartItemProps) {
  ...
  // Guest cart
  const incrementGuest = useCartStore((s) => s.incrementQuantity);
  const decrementGuest = useCartStore((s) => s.decrementQuantity);
  const removeGuest = useCartStore((s) => s.removeFromCart);

  // Server cart
  const { increment, decrement } = useUpdateCartItem();
  const { remove } = useRemoveCartItem();

  const handleIncrease = () => {
    if (!user) {
      incrementGuest(item.id);
    } else {
